...
function parseFile(file) {
  ...
  if (script.charCodeAt(0) == 0xfeff) script = script.slice(1); // Kill byte-order-mark
  return new ts.Parser().quickParse(new ts.StringSourceText(script), path.basename(file));
}
...
function translate(file) {
  definitions = {};
  var tree = parseFile(file).Script.bod;
...
console.log(JSON.stringify(translate(process.argv[2]), null, 2));
