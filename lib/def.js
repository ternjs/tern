...
(function(mod) {
  ...
  function computedFunc(name, args, retType) {
  ...
  TypeParser.prototype = {
    ...
    parseFnType: function(comp, name, top) {
      ...
      if (computed) return computedFunc(name, args, retType);
  ...
  var PromiseResolvesTo = infer.constraint({
    ...
    addType: function(tp) {
      if (tp.constructor == infer.Obj && tp.name == "Promise" && tp.hasProp("value"))
        tp.getProp("value").propagate(this.output);
      else
  ...
  infer.registerFunction("Promise_then", function(self, args, argNodes) {
    ...
    var result = new infer.Obj(infer.cx().definitions.ecma6["Promise.prototype"]);
    var value = result.defProp("value", argNodes && argNodes[0]);
    if (fn.retval.isEmpty() && self.hasProp("value"))
      self.getProp("value").propagate(value, WG_PROMISE_KEEP_VALUE);
