...
(function(root, mod) {
  ...
  function pointInProp(objNode, point) {
    for (var i = 0; i < objNode.properties.length; i++) {
      ...
      if (curProp.key.start <= point && curProp.key.end >= point)
        return {prop: curProp, type: "key"};
        if (curProp.value && curProp.value.start <= point && curProp.value.end >= point)
          return {prop: curProp, type: "value"};
    }
  }
  ...
  function findCompletions(srv, query, file) {
    ...
    if (exprAt) {
      if (exprAt.node.type == "MemberExpression" && exprAt.node.object.end < wordStart) {
        ...
        if (objProp) {
          hookname = objProp.type == "key" ? "propKeyCompletion" : "propValueCompletion";
          if (hookname == "propKeyCompletion") {
            objLit = exprAt;
            prop = objProp.prop[objProp.type].name;
          }
        } else {
          objLit = exprAt;
          prop = '';
          hookname = "propKeyCompletion";
        }
      }
    }
    ...
    if (prop != null) {
      ...
      if (!hookname) hookname = "memberCompletion";
    } else {
      infer.forAllLocalsAt(file.ast, wordStart, file.scope, gather);
      if (query.includeKeywords) jsKeywords.forEach(function(kw) {
        gather(kw, null, 0, function(rec) { rec.isKeyword = true; });
      });
      if (!hookname) hookname = "variableCompletion";
    }
  ...
  function findExprType(srv, query, file, expr) {
    ...
    if (expr.node.type == "ObjectExpression" && query.end != null &&
      ...
      var name = objProp.prop[objProp.type].name;
