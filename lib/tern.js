  ...
  function findCompletions(srv, query, file) {
    ...
    if (exprAt) {
      if (exprAt.node.type == "MemberExpression" && exprAt.node.object.end < wordStart) {
        ...
        for (var i = 0; i < exprAt.node.properties.length; i++) {
          var curProp = exprAt.node.properties[i];
          if (curProp.key.start <= wordStart && curProp.key.end >= wordEnd) {
            objLit = exprAt;
            prop = curProp.key.name;
            break;
          }
        }
      }
