...
(function(mod) {
  ...
  function isTarget(state, origin) {
    return state.origins.indexOf(origin) > -1;
  }
  ...
  function getSpan(state, node) {
    if (state.options.spans == false || !isTarget(state, node.origin)) return null;
    if (node.span) return node.span;
    var srv = state.cx.parent, file;
  ...
  function reach(type, path, id, state) {
    ...
    if (shorter) {
      ...
      if (actual.reached(newPath, state) && relevant) {
        var data = state.types[oldPath];
        if (data) {
          delete state.types[oldPath];
          state.altPaths[oldPath] = actual;
        } else data = {type: actual};
        data.span = getSpan(state, type) || (actual != type && isTarget(state, actual.origin) && getSpan(state, actual)) || data.span;
        data.doc = type.doc || (actual != type && isTarget(state, actual.origin) && type.doc) || data.doc;
  ...
  infer.Arr.prototype.reached = function(path, state) {
    reach(this.getProp("<i>"), path, "<i>", state);
    return true;
  };

  infer.Fn.prototype.reached = function(path, state) {
    infer.Obj.prototype.reached.call(this, path, state, true);
    for (var i = 0; i < this.args.length; ++i) reach(this.args[i], path, "!" + i, state);
    reach(this.retval, path, "!ret", state);
    return true;
  };

  infer.Obj.prototype.reached = function(path, state) {
    if (isSimpleInstance(this)) {
  ...
  infer.Obj.prototype.typeName = function() {
    return this.nameOverride || this.path || "?";
  };
