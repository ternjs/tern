...
(function(root, mod) {
  ...
  function reach(type, path, id, state, byName) {
    ...
    if (shorter) {
      ...
      // support Union Types
      if ((Array.isArray(actual) || actual.reached(newPath, state, !relevant)) && relevant) {
  ...
  function typeName(type) {
    Array.prototype.getType = function(guess) { return this; }
    var actual = type.getType(false);
    if (!actual || typeNameStack.indexOf(actual) > -1)
      return actual && actual.path || "?";
    typeNameStack.push(actual);
    Array.prototype.typeName = function() { return infer.toString(this,1,null); }
