...
(function(root, mod) {
  ...
  var scopeGatherer = walk.make({
    ...
    Function: function(node, scope, c) {
      var inner = node.scope = new Scope(scope)
    ...
    ForStatement: function(node, scope, c) {
      if (!node.scope && isBlockScopedDecl(node.init))
        scope = node.scope = new Scope(scopes.read)
  ...
  var inferExprVisitor = {
    ...
    FunctionExpression: ret(function(node, scope, c, name) {
      ...
      if (node.expression) {
        infer(node.body, inner, c, inner.fnType.retval = new AVal)
      } else {
        walk.recursive(node.body, inner, null, inferWrapper)
      }
  ...
  var inferWrapper = walk.make({
    Expression: function(node, scope, c) {
      infer(node, node.scope || scope, c, ANull);
    },
