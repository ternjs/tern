  ...
  var toString = exports.toString = function(type, maxDepth, parent) {
    ...
    if (Array.isArray(type)) {
      ...
      for (var i = 0; i < type.length; i++) {
        format += toString(type[i], maxDepth, parent);
        if (i < type.length-1 ) format += "|";
      }
      format += ")";
      return format;
    }
  ...
  function canonicalType(types) {
    ...
    for (var i = 0; i < types.length; ++i) {
      ...
      if (tp instanceof Arr) ++arrays;
        ...
        if (prim && tp.name != prim.name) {
          // return the Union type information, if we have nothing better to return
          return types;
          // was: return null;
        }
        prim = tp;
