...
(function(root, mod) {
  ...
  function asArgumentName(node) {
    if (node.type == "Identifier") return node.name
    if (node.type == "ObjectPattern") return "{" + node.properties.map(asArgumentName).join(", ") + "}"
    if (node.type == "ArrayPattern") return "[" + node.elements.map(asArgumentName).join(", ") + "]"
    if (node.type == "RestElement") return "..." + asArgumentName(node.argument)
  }

  var scopeGatherer = walk.make({
    VariablePattern: function(node, scopes) {
      addVar(scopes.inner, node)
    },
    ...
    Function: function(node, scope, c) {
      ...
      for (var i = 0; i < node.params.length; ++i) {
        ...
        if (param.type == "Identifier") {
          ...
          // FIXME create bindings
          var arg = new AVal
          argVals.push(arg)
          arg.originNode = param
          c(param, patternScopes(scope, inner), "Pattern")
        }
      }
    ...
    BlockStatement: function(node, scope, c) {
      if (!node.scope && node.body.some(isBlockScopedDecl))
        scope = node.scope = new Scope(scope)
