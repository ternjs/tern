...
(function(root, mod) {
  ...
  var scopeGatherer = walk.make({
    ...
    TryStatement: function(node, scope, c) {
      ...
      if (node.handler) {
        if (node.handler.param.type == "Identifier") {
          ...
          c(node.handler.param, paramScopes(scope), "Pattern")
        }
      }
    ...
    ForStatement: function(node, scope, c) {
      if (!node.scope && node.init && isBlockScopedDecl(node.init))
        scope = node.scope = new Scope(scopes.read, node, true)
      walk.base.ForStatement(node, scope, c)
    },
    ImportDeclaration: function(node, scope, c) {
  ...
  function propName(node, scope, doInfer) {
    var prop = node.property;
    if (!node.computed) return prop.name;
    if (prop.type == "Literal" && typeof prop.value == "string") return prop.value;
    if (doInfer) infer(prop, scope, ANull);
    return "<i>";
  }
  ...
  var inferPatternVisitor = {
    Identifier: function(node, scope, source) {
      source.propagate(scope.hasProp(node.name) || cx.topScope.defProp(node.name, node))
    },
    ...
    ArrayPattern: function(node, scope, source) {
      var content = source.getProp("<i>")
      for (var i = 0; i < node.elements.length; i++)
        connectPattern(node.elements[i], scope, content)
    },
  ...
  function inferClass(node, scope, name) {
    ...
    var fn = ctor ? infer(ctor, scope) : new Fn(name, ANull, [], null, ANull)
    var sup = cx.protos.Object
    if (node.superClass) {
      var supCtor = infer(node.superClass, scope).getFunctionType()
      if (supCtor) {
        var supProto = supCtor.getProp("prototype").getObjType()
        if (supProto) sup = supProto
      }
    }
    ...
    for (var i = 0; i < body.length; i++) {
      ...
      if (method.computed) infer(method.key, scope, ANull)
      if (method.computed || method.kind == "set") {
        target = ANull
      } else {
        target = (method.static ? fn : proto).defProp(method.key.name, method.key)
  ...
  var inferExprVisitor = {
    ...
    AssignmentExpression: ret(function(node, scope) {
      var rhs, name, pName;
      if (node.left.type == "MemberExpression") {
        name = pName = propName(node.left, scope, true);
        if (node.left.object.type == "Identifier")
          name = node.left.object.name + "." + name;
    ...
    CallExpression: fill(function(node, scope, out) {
      ...
      if (node.callee.type == "MemberExpression") {
        ...
        var pName = propName(node.callee, scope, true)
  ...
  function computeReturnType(funcNode, argNodes, scope) {
    var f = findType(funcNode, scope).getFunctionType()
    if (!f) return ANull
    if (f.computeRet) {
      ...
      return f.computeRet(self, args, argNodes);
    } else {
      return f.retval;
    }
  }
