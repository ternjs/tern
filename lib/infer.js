...
(function(root, mod) {
  ...
  var inferPatternVisitor = {
    ...
    MemberExpression: function(node, scope, source) {
      ...
      var pName = propName(node, scope, true)
      obj.propagate(new PropHasSubset(pName, source, node.property))
    },
  ...
  function inferClass(node, scope, name) {
    ...
    return withSuper(supCtor, delayed || sup, function() {
      ...
      for (var i = 0; i < body.length; i++) {
        ...
        var propName = method.computed ? symbolName(infer(method.key, scope)) : method.key.name
        if (!propName || method.kind == "set") {
          target = ANull
        } else {
          target = (method.static ? fn : proto).defProp(propName, method.key)
  ...
  function generatorResult(input, output) {
    ...
    var result = new Obj(cx.definitions.ecma6.generator_prototype)
