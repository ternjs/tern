...
(function(root, mod) {
  ...
  var getSymbol = exports.getSymbol = function(name, originNode) {
  ...
  function propName(node, scope, doInfer) {
    ...
    if (doInfer) {
      var symName = symbolName(infer(prop, scope))
  ...
  var inferExprVisitor = {
    ...
    ObjectExpression: ret(function(node, scope, name) {
      ...
      withSuper(null, waitForProto || proto, function() {
        for (var i = 0; i < node.properties.length; ++i) {
          ...
          if (prop.computed)
            name = symbolName(infer(key, scope))
    ...
    AssignmentExpression: ret(function(node, scope, name) {
      var rhs, pName;
      if (node.left.type == "MemberExpression") {
        pName = propName(node.left, scope, true)
        if (!name)
    ...
    MemberExpression: fill(function(node, scope, out) {
      var name = propName(node, scope), wg;
      if (name == "<i>") {
  ...
  function iteratorResult(input, output) {
  ...
  function maybeIterator(fn, output) {
    ...
    if (!fn.computeRet) { // Reuse iterator objects for non-computed return types
      if (fn.generator === true) fn.generator = iteratorResult(fn.yieldval, output)
      return fn.generator
    }
    return iteratorResult(fn.yieldval, output)
  }

  function computeReturnType(funcNode, argNodes, scope) {
    var fn = findType(funcNode, scope).getFunctionType(), result = fn.retval
    if (!fn) return ANull
    if (fn.computeRet) {
  ...
  var typeFinder = {
    ...
    YieldExpression: function(node, scope) {
      var fn = functionScope(scope).fnType
      return fn ? fn.yieldval : ANull
    }
  };
